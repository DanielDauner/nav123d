...
class PDMMetric(BaseMetric):
    ...
    def compute_metric(self, scene_api: SceneAPI, **kwargs) -> dict:
        ...
        assert "simulated_trajectory_se2" in kwargs, "Missing required argument: simulated_trajectory_se2"
        simulated_trajectory_se2 = kwargs["simulated_trajectory_se2"]
        assert isinstance(simulated_trajectory_se2, TrajectorySE2), (
            "Argument 'simulated_trajectory_se2' must be of type TrajectorySE2"
        )

        box_detections_se2_dict = kwargs["box_detections_se2_dict"]
        assert len(box_detections_se2_dict) == len(simulated_trajectory_se2.timestamps)

        # map_api = scene_api.get_map_api()

        # # replay_trajectory_se2
        # # simulated_trajectory_se2
        ...
        #     trajectory=simulated_trajectory_se2,
        #     sampling=self._score_trajectory_sampling,
        #     initial_ego_state_se2=initial_ego_state_se2,
